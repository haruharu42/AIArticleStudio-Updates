-- Phase 9 security hardening: an invite must never overwrite or shorten an existing active PWA entitlement.
...
begin
    ...
    -- The partial unique index includes expired rows while status remains active.
    -- Renew an expired active row, but never replace a still-valid or unlimited entitlement.
