-- Phase 5A follow-up: provide the complete PostgREST custom-error JSON shape.
--
-- The preceding fix established atomic row locking and a non-retrying Data API
-- boundary. PostgREST also requires string values in the message document and
-- a headers object in the detail document.
    ...
    if current_revision <> p_expected_revision then
        raise sqlstate 'PGRST' using
            message = jsonb_build_object(
                ...
                'hint', 'Reload the article before saving again.'
            )::text,
