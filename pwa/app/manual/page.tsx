...
const MANUAL_SECTIONS = [
  ...
  },
  {
    ...
    points: [
      ...
      "AASがプロンプトをコピーしてAIを開きます。AIにはAASへ貼り付ける予定表だけを返すよう指示するため、前置きや説明文を除いてコピーしやすくします。",
      "JSONを作ったり編集したりする必要はありません。AIの回答全文をそのままコピーします。",
      "AIがカレンダーへ作る予定は「無料note作成」「有料note作成」の2種類だけです。振り返り・SNS告知・初期設定は入れません。",
      "AASへ戻り「コピーしたAI回答を読み込んで反映」を押すだけで、無料note/有料note予定を自動抽出・検証・反映します。1日2回以上なら投稿数と同じ数の時刻を別々に表示します。",
      "クリップボード読取が使えない端末では、回答全文を貼り付けて「そのまま反映」を押します。",
      "貼り付け欄の内容は端末内でアカウント別に保持されます。AIを開き直したり画面を再読み込みしても消えず、「貼り付け内容をクリア」を押した時だけ削除します。",
  ...
  },
  {
    ...
    group: "サポート",
    title: "お問い合わせ・返信確認",
    when: "追加機能を要望したい時、不具合を報告したい時、使い方や利用権について管理者へ確認したい時。",
    lead: "AAS内から問い合わせを送信し、受付番号・対応状況・管理者からの返信を同じ画面で確認できます。",
    points: [
      "問い合わせ種類は「追加機能要望」「不具合・エラー」「使い方・質問」「アカウント・利用権」「購入・支払い」「その他」から選びます。",
      "問い合わせ内容は必須です。件名は任意で、発生した画面・直前の操作・期待した動作などを具体的に書くと確認しやすくなります。",
      "送信後は受付番号が発行され、問い合わせ履歴から対応状況と会話履歴を確認できます。",
      "管理者から返信が届くとホームの通知マークへ反映されます。履歴を開くと既読になります。",
      "追加情報がある場合は同じ問い合わせへ追記できます。解決した問い合わせはユーザー側から終了できます。",
      "パスワード、認証コード、アクセストークン、秘密鍵、カード番号などの秘密情報は送信しないでください。",
    ],
    links: [{ href: "/inquiries", label: "お問い合わせを開く" }],
  },
  {
    id: "creator",
    group: "活動",
  ...
  },
  {
    ...
    when: "よく使うAI、文章の好み、スマホ下部ナビの表示方法を確認したい時。",
    lead: "AASの使い勝手を自分向けに調整します。",
    points: [
      "よく使うAIと利用プランを設定します。",
      "個人最適化では、文章の好みや小さな利用傾向を次回プロンプトへ反映できます。",
      "スマホ下部ナビは全画面で同じ設定を使います。ホームは固定で、残り4枠を好きな機能へ入れ替えられます。",
      "設定の「下部ナビをカスタマイズ」から、4枠の機能選択と並び順を変更できます。activeな管理者では問合せ確認、管理ダッシュボード、ユーザー、無料設定、販売、更新管理、MFA、運用も候補に表示されます。一般ユーザーには管理項目は表示されません。",
      "初期状態は「ホーム / 作成 / ライブラリ / ランキング / プロフィール」です。4枠のカスタマイズ内容は同じ端末内でもログインユーザーごとに分けて保存されます。",
  ...
  },
  {
    ...
    group: "管理者専用",
    title: "問い合わせ確認",
    when: "ユーザーから届いた追加機能要望、不具合、質問を確認して返信・対応状況を管理する時。",
    lead: "新着・追記の未確認状態、種類、優先度、対応状況、会話履歴をまとめて管理します。",
    points: [
      "未確認の問い合わせやユーザー追記があると管理者ホームの通知マークへ反映されます。",
      "受付番号、AASユーザーID、表示名、件名で検索し、種類・状態で絞り込めます。",
      "対応状況は新規・確認中・ユーザー回答待ち・解決済み・終了で管理します。",
      "返信を送るとユーザー側の通知マークへ反映されます。",
      "優先度は通常・高・緊急から管理者だけが設定できます。",
      "パスワード、認証コード、秘密鍵、カード番号などの提出をユーザーへ求めないでください。",
    ],
    links: [{ href: "/admin/inquiries", label: "問い合わせ確認を開く" }],
  },
  {
    id: "admin-promotion",
    group: "管理者専用",
    title: "販売・プロモーションセンター",
    when: "販売前の実運用テストや開発進捗・公開予定を発信する時、販売開始後の記事やSNSキャンペーンを作る時。",
    lead: "販売前から販売後まで、現在の発信フェーズに合わせてnote等の長文記事と各SNSの投稿素材を作成します。",
    points: [
      "「テスト・公開予告」では、自分でnote・Tips・Brain等を試した確認済み内容、改善中の点、公開予定を入力し、販売前であることを明確にした記事・SNS案を作ります。",
      "価格・販売URL・公開日が未確定なら断定せず、実際に確認していないPV・売上・反応・レビュー・感想もAIに作らせません。",
      "SNSごとに目標文字数を選べます。Xは標準投稿とPremium長文を分け、必要なら自由入力の文字数も指定できます。",
      "Threadsは通常投稿用と長文テキスト添付用を分け、YouTube Shortsはタイトルと概要欄の条件を分けて扱います。",
      "Instagram・TikTokはAAS内の短文・標準・長文の目安を選び、投稿直前に各サービスの最新仕様を確認します。",
      "キャンペーン設計でも同じSNS文字数設定を使い、販売前はテスト共有・開発進捗・公開予告を中心にした14日間の計画を作れます。",
    ],
    links: [{ href: "/admin/promotion?mode=preview", label: "テスト・公開予告を開く" }],
  },
  {
    id: "admin-ops",
    group: "管理者専用",
    ...
    points: [
      ...
      "販売・プロモーションでは、確認済みの製品情報と発信フェーズを基準にテスト報告・公開予告・紹介記事・SNS素材を準備します。",
