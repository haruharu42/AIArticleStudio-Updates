  ...
  return (
    <div className="beginner-shell">
      <header className="beginner-topbar">
        <a className="beginner-brand" href="/" aria-label="AI Article Studio ホーム"><span aria-hidden="true">✦</span><strong>AI ARTICLE STUDIO</strong></a>
      ...
      {section === "home" ? (
        <main className="beginner-main">
          <section className="beginner-welcome">
            <div><p>はじめてでも迷わない記事づくり</p><h1>おかえりなさい！</h1><span>今日は何をしますか？</span></div>
            <span className="beginner-access-badge">● PWA利用可能</span>
          </section>

          <section className="beginner-recommend">
            <div><span className="beginner-recommend-label">おすすめ</span><h2>まずは記事を1本つくってみましょう</h2><p>掲載先・ジャンル・読者を選ぶだけ。順番に進めれば記事作成から画像計画まで完了できます。</p></div>
            <button type="button" onClick={() => openRoute("/create")}>記事を作成する →</button>
          </section>

          <section aria-labelledby="beginner-actions-title">
            <div className="beginner-section-heading"><div><p>MAIN MENU</p><h2 id="beginner-actions-title">やりたいことから選ぶ</h2></div><a href="/tools">すべての機能を見る</a></div>
          ...
          <section className="beginner-openai-section" aria-labelledby="beginner-openai-title">
            <div className="beginner-section-heading compact"><div><p>OPENAI</p><h2 id="beginner-openai-title">OpenAIツールを開く</h2></div></div>
            <p className="beginner-openai-help">プロンプトをコピーしたあと、そのまま公式のOpenAIツールへ移動できます。</p>
            <div className="beginner-openai-grid">
              <a href={OPENAI_LINKS.chatgpt} target="_blank" rel="noreferrer"><span aria-hidden="true">✦</span><strong>ChatGPT</strong><small>記事・タイトル・相談に</small><b>↗</b></a>
              <a href={OPENAI_LINKS.work} target="_blank" rel="noreferrer"><span aria-hidden="true">▣</span><strong>ChatGPT Work</strong><small>まとまった作業・成果物作成に</small><b>↗</b></a>
              <a href={OPENAI_LINKS.images} target="_blank" rel="noreferrer"><span aria-hidden="true">▧</span><strong>ChatGPT Images</strong><small>画像生成・画像編集に</small><b>↗</b></a>
              <a href={OPENAI_LINKS.codex} target="_blank" rel="noreferrer"><span aria-hidden="true">⌘</span><strong>Codex</strong><small>コード作成・開発作業に</small><b>↗</b></a>
            </div>
          ...
          <section className="beginner-secondary-section" aria-labelledby="beginner-more-title">
            <div className="beginner-section-heading compact"><div><p>MORE</p><h2 id="beginner-more-title">その他の機能</h2></div></div>
          ...
          <section className="beginner-account-panel">
            <div><strong>{profile.display_name || "ユーザー"}</strong><span>{profile.aas_user_id} · {profile.role} / {profile.status}</span></div>
            <div><button type="button" onClick={() => void refresh(client)}>利用権を再確認</button><button className="danger" type="button" onClick={() => void logout()}>ログアウト</button></div>
          </section>
        </main>
      ) : (
