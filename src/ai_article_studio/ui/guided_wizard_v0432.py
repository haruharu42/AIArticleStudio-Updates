...
def _decorate_completion_preview(widget):
    try:
        widget.tag_configure("v0432_h1", font=("Yu Gothic UI", 16, "bold"), foreground="#111827", spacing1=12, spacing3=8)
        widget.tag_configure("v0432_h2", font=("Yu Gothic UI", 13, "bold"), foreground="#5B21B6", spacing1=10, spacing3=6)
        widget.tag_configure("v0432_h3", font=("Yu Gothic UI", 11, "bold"), foreground="#1D4ED8", spacing1=8, spacing3=4)
        widget.tag_configure("v0432_quote", foreground="#374151", background="#EDE9FE", lmargin1=14, lmargin2=14, spacing1=4, spacing3=4)
        widget.tag_configure("v0432_marker", foreground="#5B21B6", background="#EDE9FE", font=("Yu Gothic UI", 10, "bold"), lmargin1=12, lmargin2=12, spacing1=7, spacing3=7)
        content = widget.get("1.0", "end-1c")
        for number, line in enumerate(content.splitlines(), start=1):
            start, end = f"{number}.0", f"{number}.end"
            stripped = line.lstrip()
            if stripped.startswith("# "):
                widget.tag_add("v0432_h1", start, end)
            elif stripped.startswith("## "):
                widget.tag_add("v0432_h2", start, end)
            elif stripped.startswith("### "):
                widget.tag_add("v0432_h3", start, end)
            elif stripped.startswith("> "):
                widget.tag_add("v0432_quote", start, end)
            elif re.match(r"^\[挿絵[1-9]\d*｜", stripped):
                widget.tag_add("v0432_marker", start, end)
    except Exception:
        pass
        ...
        for widget in existing:
            for item in _walk(widget):
                try:
                    ...
                    if matches(text):
                        app._v0432_creation_action = text
                        item.invoke()
                        return True
                except Exception:
                    pass
...
def _install_history_panel_10(app, on_load=None, on_delete=None):
    ...
    panel = getattr(app, "_v0432_history_panel", None)
    if panel is None or not panel.winfo_exists():
        ...
        app._v0432_history_panel = panel
        app._v0432_history_list = listing
        old = getattr(app, "_v0427_history_panel", None)
        if old is not None and old is not panel:
            _hide(old)
    listing = app._v0432_history_list
...
def install_article_wizard(app, body):
    ...
    def start_new():
        try:
            ...
            pass
        state.update({"phase": 0, "title_prompt": "", "article_prompt": "", "candidates": [], "source_article": "", "insertion_article": "", "article": ""})
    ...
    def clear_paste_fields():
        for key in ("title_response", "article_response"):
            ...
            if widget is not None:
                _set_text(widget, "")
        state.update({"source_article": "", "insertion_article": "", "article": ""})
        try:
    ...
    def load_history(article_id):
        ...
        source = str(snapshot.get("normalized_output") or snapshot.get("raw_web_output") or snapshot.get("formatted_output") or "")
        variants = build_article_text_variants(source)
        state["source_article"] = variants.source_text
        state["insertion_article"] = variants.insertion_text
        state["article"] = str(snapshot.get("formatted_output") or variants.publish_text)
        if state["article"]:
            render_completion()
            show_page(5)
        else:
    ...
    def prepare_embedded_creation():
        ...
        try:
            ...
            return False
        state.update({"phase": 0, "title_prompt": str(result.get("prompt") or ""), "article_prompt": "", "candidates": [], "source_article": "", "insertion_article": "", "article": ""})
        fields["selected_title"].set("")
        render_creation()
        show_page(4)
        refresh_history()
    ...
    def render_completion():
        ...
        _label(left, "紫色の行が画像の差し込み位置です。公開用コピーでは自動的に除外されます。", size=8, fg=SOFT).pack(anchor="w", padx=16, pady=(0, 8))
        ...
        _set_text(preview, insertion_article)
        ...
        _primary(app, actions, "掲載用をコピー", lambda: _copy(app, article, "挿絵マーカーを除いた掲載用記事をコピーしました。" )).pack(side="left")
        _secondary(app, actions, "画像差し込み用", lambda: _copy(app, insertion_article, "画像の差し込み位置を残してコピーしました。" )).pack(side="left", padx=7)
        _secondary(app, actions, "元記事", lambda: _copy(app, source_article, "取り込んだ元記事をコピーしました。" )).pack(side="left")
    ...
    def complete_article_phase():
        ...
        platform = str(_request_snapshot(app).get("platform") or "note")
        try:
            publish_result = app.web_ai_bridge.publish_step(article, platform=platform)
        except Exception as exc:
    ...
    app._v0432_activation_marker = ACTIVATION_MARKER
    refresh_history()
    show_page(0)
    return root
...
def activate_live_article_wizard(app):
    ...
    if body is None:
        ...
        app._v0432_activation_error = "live_article_body_not_found"
        return None
    ...
    wizard = getattr(app, "_v0432_article_wizard", None)
