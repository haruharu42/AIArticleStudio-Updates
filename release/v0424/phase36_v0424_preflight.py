...
ARTICLE_PROMPT_ANCHOR = "self.web_ai_bridge.build_article_step"
...
def inspect(app_root: Path) -> dict:
    ...
    prompt = core / "image_prompt_builder.py"
    settings = core / "image_settings.py"
    ...
    result = {
        ...
        "image_core_exists": all(path.is_file() for path in (prompt, settings, engine, workflow)),
        "phase35_ui": False,
        "phase36_ui": False,
        "v0422_ui": False,
        "v0423_style": False,
        "article_prompt_anchor": False,
        "safe_to_patch": False,
        "reason": "unknown",
    }
    if app.is_file():
        ...
        result["article_prompt_anchor"] = ARTICLE_PROMPT_ANCHOR in text
    if prompt.is_file():
        prompt_text = prompt.read_text(encoding="utf-8")
        result["v0423_style"] = all(token in prompt_text for token in ("STYLE_RULES", "セル塗り", "企業広告イラスト"))
    if not app.is_file() or not init.is_file() or not result["image_core_exists"]:
    ...
    elif not all((result["phase35_ui"], result["phase36_ui"], result["v0422_ui"], result["v0423_style"], result["article_prompt_anchor"])):
        result["reason"] = "required_v0423_marker_missing"
    else:
        result["safe_to_patch"] = True
        result["reason"] = "canonical_v0423_verified"
    return result
