...
VERSION = "0.4.2.8"
PACKAGE_NAME = "AIArticleStudio_Update_v0.4.2.8_DirectVisualWizard.zip"
PACKAGE_PATH = ROOT / "updates" / PACKAGE_NAME
STAGE = ROOT / ".build_v0428"
RELEASE_FILES = (
    "README.txt",
    "Update.ps1",
    "phase36_v0428_preflight.py",
    "patch_v0428.py",
    "set_version_v0428.py",
    "validate_v0428.py",
)
...
def main() -> None:
    ...
    for name in RELEASE_FILES:
        shutil.copy2(ROOT / "release" / "v0428" / name, STAGE / name)
    ...
    manifest = {
        ...
        "sha256": digest,
        "channel": "stable",
        "notes": "v0.4.2.8: directly activates the approved six-step visual wizard, removes legacy call paths, and adds real Windows Tkinter runtime validation.",
    }
    (ROOT / "candidate-v0428.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "release" / "v0428" / "SHA256.txt").write_text(digest + "  " + PACKAGE_NAME + "\n", encoding="ascii")
