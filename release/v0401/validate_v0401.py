...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: validate_v0401.py <AIArticleStudio install root>")
    ...
    if '__version__ = "0.4.0.1"' not in init_text:
        fail("version is not v0.4.0.1")
    for marker in REQUIRED_APP_MARKERS:
        if marker not in app_text:
            fail(f"missing app marker: {marker}")
    if app_text.count("# v0.4.0 Phase 3.5 integrated Web AI") != 2:
        fail("Phase 3.5 marker count is unexpected")
    ast.parse(app_text, filename=str(app))
    for name in REQUIRED_CORE:
        path = core / name
        if not path.is_file():
            fail(f"missing bridge core file: {name}")
        ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
    workflow = (core / "web_ai_workflow.py").read_text(encoding="utf-8")
    if "build_title_prompt_v2" not in workflow or "build_final_article_prompt_v2" not in workflow:
        fail("Web Prompt Engine v2 workflow hooks are missing")
    print("V0.4.0.1 BRIDGE FEATURE VALIDATION OK")
