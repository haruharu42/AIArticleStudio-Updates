...
VERSION = "0.4.2.7"
PACKAGE_NAME = "AIArticleStudio_Update_v0.4.2.7_VisualSixStepWizard.zip"
PACKAGE_PATH = ROOT / "updates" / PACKAGE_NAME
STAGE = ROOT / ".build_v0427"
RELEASE_FILES = (
    "README.txt",
    "Update.ps1",
    "phase36_v0427_preflight.py",
    "patch_v0427.py",
    "set_version_v0427.py",
    "validate_v0427.py",
)
FIXED_DATE = (2026, 8, 21, 0, 0, 0)
...
def main() -> None:
    ...
    for name in RELEASE_FILES:
        shutil.copy2(ROOT / "release" / "v0427" / name, STAGE / name)
    ...
    manifest = {
        ...
        "sha256": digest,
        "channel": "stable",
        "notes": "v0.4.2.7: dedicated six-step visual article wizard, persistent recent work, explicit paste clearing, decorated completion preview, and inline image-prompt results.",
    }
    (ROOT / "candidate-v0427.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "release" / "v0427" / "SHA256.txt").write_text(digest + "  " + PACKAGE_NAME + "\n", encoding="ascii")
    with zipfile.ZipFile(PACKAGE_PATH) as archive:
        bad = archive.testzip()
        if bad:
            raise RuntimeError(f"ZIP CRC failure: {bad}")
        required = set(RELEASE_FILES) | {"payload/ui/guided_wizard_v0427.py"}
