...
VERSION = "0.4.3.0"
PACKAGE_NAME = "AIArticleStudio_Update_v0.4.3.0_StepFiveAIAuto.zip"
PACKAGE_PATH = ROOT / "updates" / PACKAGE_NAME
STAGE = ROOT / ".build_v0430"
RELEASE_FILES = (
    "README.txt",
    "Update.ps1",
    "phase36_v0430_preflight.py",
    "patch_v0430.py",
    "cleanup_v0430.py",
    "set_version_v0430.py",
    "validate_v0430.py",
)
PAYLOAD_MODULES = (
...
def main() -> None:
    ...
    for name in RELEASE_FILES:
        shutil.copy2(ROOT / "release" / "v0430" / name, STAGE / name)
    ...
    manifest = {
        ...
        "sha256": digest,
        "channel": "stable",
        "notes": "v0.4.3.0: fixes STEP 4 to STEP 5 using the validated original creation action, adds AI-auto theme selection, and safely refreshes Python caches.",
    }
    (ROOT / "candidate-v0430.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "release" / "v0430" / "SHA256.txt").write_text(digest + "  " + PACKAGE_NAME + "\n", encoding="ascii")
